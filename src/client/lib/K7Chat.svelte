          ...
          {#each messages as m (m.id)}
            ...
            {:else if m.content || streaming}
              <p class="line line-{m.role}">
                ...
                <!-- renderMarkdown emits only its own tag allowlist and escapes any raw HTML in
                     the message (see shared/markdown.ts), and linkDurations only wraps text
                     between those tags — this is not raw model/user output reaching the DOM. -->
...
<style>
  ...
  /* Every heading level renders at h3–h6 (lib/markdown.ts); in a chat bubble
     they are section labels, not document titles — one size, set apart by
     weight and the label tracking, never larger than the answer's own text. */
  .content :global(h3),
  .content :global(h4),
  .content :global(h5),
  .content :global(h6) {
  ...
  .content :global(:is(h3, h4, h5, h6):first-child) {
    margin-top: 0;
  }

  /* A loose list (items separated by blank lines) wraps each item in <p>. */
  .content :global(li > p) {
  ...
  .content :global(.md-table) {
    margin: 0 0 var(--space-2);
    overflow-x: auto;
    -webkit-overflow-scrolling: touch;
  }
  .content :global(.md-table:last-child) {
    margin-bottom: 0;
  }
  ...
  .content :global(.al-c) { text-align: center; }
  .content :global(.al-r) { text-align: right; }
